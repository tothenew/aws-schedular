...
import boto3
from global_variables import SLACK_URL, ACCESS_KEY, SECRET_KEY
    ...
    if file_check:
        for res in resources:
            if res in config_workspace:
                if res == "rds":
                    rds_instace_list = rds_module.schedule_rds(config_workspace[res], action)
                if res == "asg":
                    asg_scheduler_resp = asg_module.main_schedular_asg(config_workspace[res], action)

        with ThreadPoolExecutor(max_workers=10) as executor:
            for result in executor.map(rds_module.check_db_status, rds_instace_list):
                rds_scheduler_resp = result
        data = "--------Below are the details about RDS-------\n"
        if len(rds_scheduler_resp) > 0:
            print(rds_scheduler_resp)
